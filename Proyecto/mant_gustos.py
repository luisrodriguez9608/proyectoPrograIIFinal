...
from modelo import Gusto

#Incluye el objeto de logica de negocio
from modelo import GustoBO
...
class Aplicacion:
    ...
    def __init__(self):
        ...
        #*************************************************************************
        self.gusto = Gusto.Gusto() #se crea el objeto de dominio para guardar la información
        ...
        self.lb_id = Label(self.raiz, text = "ID:")
        self.lb_id.place(x = 240, y = 60)
        self.txt_id = Entry(self.raiz, textvariable=self.gusto.id, justify="right")
        self.txt_id.place(x = 370, y = 60)
        ...
        self.txt_nombre = Entry(self.raiz, textvariable=self.gusto.nombre, justify="right", width=30)
        ...
        self.lb_descripcion.place(x = 240, y = 120)
        self.txt_descripcion = Entry(self.raiz, textvariable=self.gusto.descripcion, justify="right", width=30)
        self.txt_descripcion.place(x = 370, y = 120)
        ...
        self.txt_cedula = Entry(self.raiz, textvariable=self.gusto.cedula, justify="right", width=30)
        self.txt_cedula.place(x = 370, y = 180)
        ...
        self.sheet = Sheet(self.raiz,
                           ...
                           #header_align = "w",
                            ...
                            headers = ['ID', 'Nombre', 'Descripcion', 'Cedula'],
    ...
    def cargarTodaInformacion(self):
        try:
            self.gustoBO = GustoBO.GustoBO() #se crea un objeto de logica de negocio
            resultado = self.gustoBO.consultar()
    ...
    def cargarInformacion(self):
        try:
            ...
            id = (self.sheet.get_cell_data(datoSeleccionado[0],0))
            self.gusto.id.set(id)
            self.gustoBO = GustoBO.GustoBO() #se crea un objeto de logica de negocio
            self.gustoBO.consultarGusto(self.gusto) #se envia a consultar
            self.insertando = False
            msg.showinfo("Acción: Consultar gusto", "La información de la persona ha sido consultada correctamente") # Se muestra el mensaje de que todo esta correcto
    ...
    def eliminarInformacion(self):
        try:
            ...
            if resultado == "yes":
                self.gusto.id.set(id)
                self.gustoBO = GustoBO.GustoBO() #se crea un objeto de logica de negocio
                self.gustoBO.eliminar(self.gustoBO) #se envia a consultar
                self.cargarTodaInformacion()
                self.gusto.limpiar()
        except Exception as e: 
            msg.showerror("Error",  str(e)) #si se genera algun tipo de error muestra un mensache con dicho error
    ...
    def enviarInformacion(self):
        try:
            ...
            if(self.insertando == True):
                self.gustoBO.guardar(self.gusto)
            else:
                self.gustoBO.modificar(self.gusto)
            
            self.cargarTodaInformacion()
            self.gusto.limpiar() #se limpia el formulario

            if(self.insertando == True):
                msg.showinfo("Acción: Agregar gusto", "La información de la persona ha sido incluida correctamente") # Se muestra el mensaje de que todo esta correcto
            else:
    ...
    def limpiarInformacion(self):
        self.gusto.limpiar() #llama al metodo de la clase persona para limpiar los atritudos de la clase
        self.insertando = True
